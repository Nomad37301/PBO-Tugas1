...
private static void tampilkanSaham(List<Saham> sahamList) {
    ...
    public static void hapusSaham (Scanner scanner){
        List<Saham> list = Database.getSahamList();
        if (list.isEmpty()) {
        ...
        int index = MenuUtil.getIntegerInput(scanner, "Pilih indeks saham untuk dihapus: ", 1, list.size());
        Saham removed = list.remove(index - 1);
        System.out.println("✅ Saham " + removed.getNamaPerusahaan() + " berhasil dihapus.\n");
    }
}
